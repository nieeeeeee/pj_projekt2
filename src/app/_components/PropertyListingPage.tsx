...
export default function PropertyListingPage({ data }: { data: PropertyData }) {
  ...
  const handleDateChange = (dates: [Date | null, Date | null]) => {
    ...
    if (start && end) {
      const isRentedDateInRange = rentedDates.some(rentedDate =>
        rentedDate >= start && rentedDate <= end
      );
      ...
      Cookies.set('selectedDates', JSON.stringify([start.toISOString(), end.toISOString()]), { expires: 7 });
    }
  ...
  const handleSaveDates = () => {
    if (selectedDates[0] && selectedDates[1]) {
      Cookies.set('selectedDates', JSON.stringify([selectedDates[0].toISOString(), selectedDates[1].toISOString()]), { expires: 7 });
  ...
  return (
    <div className="container py-5 bg-light">
      ...
      <div className="bg-white p-4 rounded mb-4">
        ...
        <div className="row">
          <div className="col-6"><strong>Powierzchnia:</strong> {data.size}</div>
          <div className="col-6"><strong>Pokoje:</strong> {data.rooms} pokoi</div>
          <div className="col-6"><strong>Rodzaj zabudowy:</strong> {data.type}</div>
          <div className="col-6"><strong>Stan:</strong> {data.status}</div>
          <div className="col-6"><strong>Ogrzewanie:</strong> {data.heating}</div>
          <div className="col-6"><strong>Ogłoszeniodawca:</strong> {data.landlord}</div>
        </div>
      </div>
